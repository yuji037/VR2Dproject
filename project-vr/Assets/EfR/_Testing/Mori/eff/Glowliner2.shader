...
#pragma target 3.0
		...
		void surf(Input IN, inout SurfaceOutput o)
		{
			...
			//coord = coord * 2.0 - 1.0;
			coord.x *= IN.worldPos.x / IN.worldPos.y;
			coord += n.xz * scale + float2(0.0, _Time.y*0.1);
			float2 cid2 = floor(coord);
			float cid = (cid2.y*10.0 + cid2.x)*0.1;

			float2 dg = circuit(coord);
			float d = dg.x;
			float3 col1 = (0.2 - (max(min(d, 2.0) - 1.0, 0.0)*2.0*0.25)) * float3(1.0, 1.1, 1.3);
			float3 col2 = (max(d - 1.0, 0.0)*2.0*0.5) * float3(1.0, 1.2, 1.6);

			float f = max(0.5 - fmod(coord.y - coord.x*0.3 + (_Time.y*0.4) + (dg.y*0.4), 2.5), 0.0)*5.0;
			col2 *= f;

			o.Emission = float3(col1 + col2);
		}
		ENDCG
		}
			FallBack "Diffuse"
